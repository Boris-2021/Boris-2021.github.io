function transparent_header(){
    if($(window).scrollTop()<parseInt(window.innerHeight*0.12)){
        $(".kayden-header > .header-inside").addClass("transparent_header")}else{
        ...
        $preloader.fadeOut(700,function(){$("body").removeAttr("style");
        a.dispatchEvent(b)})
    }transparent_header();
    loading();
    window.addEventListener("load",function(a){loaded(document.body)});